...
CREATE TABLE Drive (
    ...
    Drive_End_Date DATE,
    Drive_Status VARCHAR(20) DEFAULT 'Upcoming'
    CHECK (Drive_Status IN ('Upcoming', 'Ongoing', 'Completed'))
);
...
CREATE TABLE register (
    ...
    Drive_ID VARCHAR(10) REFERENCES Drive(Drive_ID) ON DELETE CASCADE,
...
CREATE TABLE Alumni (
    ...
    CONSTRAINT fk_alumni_student
        ...
        -- ON DELETE CASCADE ensures that if a student record is deleted, 
        -- the corresponding alumni record is also deleted.
        ON DELETE CASCADE
);
...
CREATE TABLE Company (
    ...
    Year_Founded INT,
    Last_Visit_Year INT, -- Changed from Last_Visit_Date
...
CREATE TABLE Job_Role (
    ...
    Drive_ID VARCHAR(10) NOT NULL REFERENCES Drive(Drive_ID) ON DELETE CASCADE,
...
Register_ID INT REFERENCES register(Register_ID) ON DELETE CASCADE,
...
CREATE TABLE Selection_Round (
   ...
   Round_Type VARCHAR(50) NOT NULL CHECK (Round_Type IN
       ('Aptitude Test', 'Technical Test',
        'Technical Interview', 'HR Interview', 'Case Study',
        'Coding Test')
   ),
   Duration_Minutes INT,
      Cutoff_Score DECIMAL(5,2),  -- Minimum score to pass
...
CREATE TABLE Student_Round_Result (
 ...
 Round_ID INT REFERENCES Selection_Round(Round_ID) ON DELETE CASCADE, Application_ID INT REFERENCES Application(Application_ID) ON DELETE CASCADE, 
...
CREATE TABLE Offer_Letter (
   ...
   S_ID VARCHAR(10) NOT NULL,
   Job_Role_ID VARCHAR(10) NOT NULL,
   ...
   CONSTRAINT fk_job_role
       ...
       ON DELETE CASCADE,
   CONSTRAINT unique_student_job_offer
       UNIQUE (S_ID, Job_Role_ID)
);
